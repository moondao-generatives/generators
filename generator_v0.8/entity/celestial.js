...
!(function() {
	...
	var 
		...
		ANIMINTERVAL_R = 4200, // Rotation duration scale in ms
		ANIMINTERVAL_P = 4000, // Projection duration in ms
		ANIMINTERVAL_Z = 1000, // Zoom duration scale in ms
	...
	if (typeof module === "object" && module.exports) {
		var d3js = require('d3.js'),
			d3_geo_projection = require('d3.geo.projection.js');
		module.exports = {
			Celestial: function() {
				return Celestial;
			},
	...
	// Transform variables
	var τ = Math.PI * 2, halfπ = Math.PI / 2, deg2rad = Math.PI / 180;
	...
	function transform(c, euler) {
		var x, y, z, β, γ, λ, φ, dψ, ψ, θ, ε = 1.0e-5;
	...
	var bvcolor =
		d3.scale.quantize().domain([3.347, -0.335]).range(['#ff4700', '#ff4b00', '#ff4f00', '#ff5300', '#ff5600', '#ff5900', '#ff5b00', '#ff5d00', '#ff6000', '#ff6300', '#ff6500', '#ff6700', '#ff6900', '#ff6b00', '#ff6d00', '#ff7000', '#ff7300', '#ff7500', '#ff7800', '#ff7a00', '#ff7c00', '#ff7e00', '#ff8100', '#ff8300', '#ff8506', '#ff870a', '#ff8912', '#ff8b1a', '#ff8e21', '#ff9127', '#ff932c', '#ff9631', '#ff9836', '#ff9a3c', '#ff9d3f', '#ffa148', '#ffa34b', '#ffa54f', '#ffa753', '#ffa957', '#ffab5a', '#ffad5e', '#ffb165', '#ffb269', '#ffb46b', '#ffb872', '#ffb975', '#ffbb78', '#ffbe7e', '#ffc184', '#ffc489', '#ffc78f', '#ffc892', '#ffc994', '#ffcc99', '#ffce9f', '#ffd1a3', '#ffd3a8', '#ffd5ad', '#ffd7b1', '#ffd9b6', '#ffdbba', '#ffddbe', '#ffdfc2', '#ffe1c6', '#ffe3ca', '#ffe4ce', '#ffe8d5', '#ffe9d9', '#ffebdc', '#ffece0', '#ffefe6', '#fff0e9', '#fff2ec', '#fff4f2', '#fff5f5', '#fff6f8', '#fff9fd', '#fef9ff', '#f9f6ff', '#f6f4ff', '#f3f2ff', '#eff0ff', '#ebeeff', '#e9edff', '#e6ebff', '#e3e9ff', '#e0e7ff', '#dee6ff', '#dce5ff', '#d9e3ff', '#d7e2ff', '#d3e0ff', '#c9d9ff', '#bfd3ff', '#b7ceff', '#afc9ff', '#a9c5ff', '#a4c2ff', '#9fbfff', '#9bbcff']);
	...
	function form(cfg) {
		...
		for (var fld in names) {
			...
			if (keys.length > 1) {
				...
				col.append("input").attr("type", "checkbox").attr("class", "checkboxInput").attr("id", "stars-" + fld).property("checked", config.stars[fld]).on("change", apply);
		...
		col.append("label").attr("for", "dsos-limit").html("down to magnitude ");
		...
		for (fld in names) {
			...
			col.append("label").attr("for", "dsos-" + fld).html(" ");
	...
	// Activate/Deactivate fields depending on selection of dependencies
	...
	function geo(cfg) {
		var dtFormat = d3.time.format("%Y-%m-%d %H:%M:%S"),
			zenith = [0, 0],
			geopos = [7.1315, 171.1845], // Marshall Ilands coordinates
			date = new Date(),
		...
		*/
		...
		col.append("label").attr("title", "Location coordinates long/lat").attr("for", "lat").attr("id", "LocationLabel").html("Location ");
		...
		col.append("label").attr("title", "Local date/time").attr("for", "datetime").attr("id", "dateTimeLabel").html(" Date/time ");
		...
		col.append("label").attr("title", "Show solar system objects").attr("for", "planets-show").attr("id", "planets-show-label").html(" Planets,Sun and Moon");
	...
	function exportSVG(fname) {
		...
		function svgSkyStyle(dist, pt) {
			...
			return {
				...
				"fill-opacity": skyTransparency(factor, 0.4)
			};
		}
		...
		function planetSize(m) {
			var mag = m || 2;
			var r = 8 * adapt * Math.exp(-0.05 * (mag + 2));
			return Math.max(r, 2);
		}
